 ...
 *     读取一个文件并返回：
 *       - Markdown/文本（.md .markdown .mdown .mkd .txt）：{ ok, kind:'text', path, dir, name, size, mtime, content }
 *       - 图片（png jpg jpeg gif webp avif bmp ico svg）：{ ok, kind:'image', mime, data(base64) }
 *       - HTML（.html .htm）：{ ok, kind:'html', path, dir, name, size, mtime, content }
 *     meta=1 时只返回元信息（不含内容，供自动跟随轮询）。
 *
 *   GET /api/md-reader/list?dir=<目录>
 *     列出一个目录下的 Markdown / HTML 文件（供正文内文件提及的同步解析），
 *     返回 { ok, dir, entries: [{ name, path }], truncated }。
 *
 * 安全边界：
 *   - 扩展名白名单：.md .markdown .mdown .mkd .txt .html .htm + 常见图片格式；
	...
	apply(ctx) {
		...
		async function handleFile(request) {
			try {
				...
				if (!mime && !isHtml && !hasMarkdownExtension(abs)) {
					return fail(415, '仅支持阅读 Markdown/文本（.md .markdown .mdown .mkd .txt）、HTML（.html .htm）与图片（png jpg jpeg gif webp avif bmp ico svg）');
				}
				let info;
				try {
					info = await stat(abs);
				...
				const meta = {
					ok: true,
					kind: mime ? 'image' : isHtml ? 'html' : 'text',
					path: abs,
					dir: dirname(abs),
					name: basename(abs),
					size: info.size,
		...
		async function handleList(request) {
			try {
				...
				const entries = dirents
					.filter((entry) => entry.isFile() && (hasMarkdownExtension(entry.name) || HTML_EXTENSIONS.has(extname(entry.name).toLowerCase())))
