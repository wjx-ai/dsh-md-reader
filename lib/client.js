window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const READER_PATH_PATTERN = /\.(md|markdown|mdown|mkd|png|jpe?g|gif|webp|svg|bmp|ico|avif)$/i;
			...
			try {
				if (document.getElementById("dsh-md-reader-style") === null) {
					...
					style.textContent = [
						...
						".dsh-md-reader-body th,.dsh-md-reader-body td{min-width:0!important;max-width:none!important;word-break:break-word}",
						".dsh-md-reader-body table{max-width:none}"
					].join("\n");
		...
		const MdReaderPanel = react.memo(function MdReaderPanel(props) {
			...
			const hasToc = !isImage && outline.length > 0;
